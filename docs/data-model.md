...
本文件为设计提案；SQL 是可执行的建表定义，不表示完整存储服务已实现。
...
数据库文件使用上述名称，不带扩展名。代码仓库与用户数据目录分开。音频、模型与克隆提示存文件，不以 Base64 塞进 JSON；body 存 asset ID、相对路径、hash、格式、时长和状态。路径解析须限制在配置的本地数据目录，外部导入先复制为受管资产。
