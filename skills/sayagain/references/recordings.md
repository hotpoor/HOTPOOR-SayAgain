...
CAMPPlus and Zipformer are optional models registered for later features. The current UI does not implement diarization, speaker clustering, keyword configuration or background listening. Verify their individual inference separately before marking their descriptors verified; report that separately from product integration. Keep recordings local, and do not silently switch to cloud if local inference fails. Machine transcripts must be reviewed; do not send uncertain recognized speech directly to expression correction as if user-confirmed.
