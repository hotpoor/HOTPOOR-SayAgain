...
The app validates spans, categories, configuration snapshots and decision consistency. `needs_improvement` requires 1–20 suggestions; all other decisions require an empty `expressions` array. Categories: `grammar`, `word_choice`, `naturalness`, `register`, `translation_practice`. Confidence is a finite number between 0 and 1.
