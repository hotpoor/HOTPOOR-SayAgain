...
Cloud mode remains optional and separately configured with the user's chosen provider and credentials. Never silently upload audio or text. See `<repo>/docs/speech.md` for setup commands and implementation limits; `<repo>` is this skill directory's `../..`.
