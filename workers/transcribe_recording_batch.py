...
def main():
    ...
    with contextlib.redirect_stdout(sys.stderr):
        ...
        for clip in request['clips']:
            language=clip.get('language','auto')
            if language not in recognizers:recognizers[language]=sherpa_onnx.OfflineRecognizer.from_sense_voice(model=str(next(folder.glob('*int8.onnx'))),tokens=str(folder/'tokens.txt'),num_threads=4,use_itn=True,language=language)
