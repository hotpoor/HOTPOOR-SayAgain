...
(async()=>{try{
 ...
 const cropCanvas=page.locator('.avatar-crop-dialog canvas'),cropBox=await cropCanvas.boundingBox();await page.mouse.move(cropBox.x+cropBox.width/2,cropBox.y+cropBox.height/2);await page.mouse.down();await page.mouse.move(cropBox.x+cropBox.width/2+30,cropBox.y+cropBox.height/2+20,{steps:4});await page.mouse.up();
 await page.locator('.avatar-crop-dialog canvas').press('ArrowRight');await page.locator('[data-crop-confirm]').click();
 ...
 const browsePosition=await slider.inputValue();await page.locator('[data-chat-audio]').last().evaluate(el=>{el.dispatchEvent(new Event('timeupdate'));});assert(await page.locator('[data-playback-marker]').first().evaluate(el=>parseFloat(el.style.top)>90));assert.equal(await slider.inputValue(),browsePosition);
