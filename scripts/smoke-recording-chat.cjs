...
(async()=>{try{
 ...
 await page.locator('[data-avatar-preview] img').first().waitFor();await page.locator('[data-speaker-form="A"] button[type=submit]').click();await page.locator('.speaker-run-identity img').first().waitFor();assert(await page.locator('.reader-avatar-stack img').count());assert(await page.locator('.source-people img').count());
 ...
 const play=page.locator('[data-chat-play]').first();await play.click();await page.waitForFunction(()=>document.querySelector('[data-chat-audio]').currentTime>0.15);await play.click();assert(await page.locator('.chat-playback').first().isVisible());
