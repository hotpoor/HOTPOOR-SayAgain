...
const {modelStatus,MODEL,REVISION}=require('../desktop/local-model.cjs');
...
try{
 ...
 execFileSync(uv,['pip','install','--python',python,'-r',path.join(__dirname,'../workers/requirements.txt')],{stdio:'inherit',env:{...process.env,UV_CACHE_DIR:path.join(root,'cache')}});
