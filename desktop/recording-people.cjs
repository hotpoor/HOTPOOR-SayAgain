  ...
  return this.store.transaction(()=>{
   ...
   if(input.action==='unlink'){
    ...
    if(input.person_id){person=this.entity(input.person_id,'recording_person');if(person.body.profile_id!==this.profileId)throw Error('人物不可用');}
     ...
     person=target.person_id?this.entity(target.person_id,'recording_person'):this.putRecordingPerson({type:'recording_person',profile_id:this.profileId,name:target.name||view.name(session,input.target_key),note:target.note||'',avatar:target.avatar||''});
     ...
     person=this.putRecordingPerson({type:'recording_person',profile_id:this.profileId,name:input.name.trim(),note:input.note.trim(),avatar});
