...
module.exports=Service=>{
 Service.prototype.linkRecordingPerson=function(input){
  return this.store.transaction(()=>{
   ...
   const assign=(key,person)=>{const i=profiles.findIndex(p=>p.key===key),old=i<0?{key,name:'',note:''}:profiles[i];const value={...old,person_id:person?.block_id||null,avatar_override:person?(old.avatar_override||old.avatar||null):null};if(i<0)profiles.push(value);else profiles[i]=value;};
   let person;
   if(input.action==='unlink'){
    const oldPerson=previous.person_id?this.entity(previous.person_id,'recording_person'):null;
    assign(input.key,null);
    if(oldPerson){const i=profiles.findIndex(p=>p.key===input.key);profiles[i]={...profiles[i],name:oldPerson.body.name,note:oldPerson.body.note,avatar:previous.avatar_override||oldPerson.body.avatar||''};}
   }
   else {
    if(input.person_id){person=this.entity(input.person_id,'recording_person');if(person.body.profile_id!==this.profileId)throw Error('人物不可用');}
     ...
     person=target.person_id?this.entity(target.person_id,'recording_person'):this.store.put({type:'recording_person',profile_id:this.profileId,name:target.name||view.name(session,input.target_key),note:target.note||'',avatar:target.avatar||''});
     ...
     if(typeof avatar!=='string'||avatar.length>400000||(avatar&&!/^data:image\/(png|jpeg|webp);base64,[A-Za-z0-9+/]+={0,2}$/.test(avatar)))throw Error('头像格式无效');
     person=this.store.put({type:'recording_person',profile_id:this.profileId,name:input.name.trim(),note:input.note.trim(),avatar});
    }else throw Error('请选择人物或新建人物');
    const localAvatar=previous.avatar_override||previous.avatar;
