...
async function copyableSkill(){
 ...
 await walk(source);
 const introduction='# SayAgain 完整 Skill 包\n\n请将下列文件按相对路径保存到一个 sayagain 文件夹，再读取 SKILL.md。若需要安装，请使用你的客户端支持的 Skill 目录；不要覆盖已有同名 Skill，应先比较内容。所有所需脚本与协议都在下方，无需访问原仓库。SayAgain 必须在执行命令的同一台电脑运行，并启用本地接口。需要 Node.js 22 或更新版本。先执行 scripts/client.cjs context 验证连接；尚未授权安装模型或启用全局 hook。\n\n';
