...
> Electron 源码客户端 v0.2：设置顶部提供多个 Qwen AK 的命名、查看、切换、本地明文保存和平台快捷链接；已接入音色克隆与合成任务流程、四款云端模型选择、Skill 本地评估接口。Qwen3-TTS VC 与 Qwen-Audio 3.0 TTS Plus 已完成真实合成，本地模型推理尚未验证；尚无安装包。
...
空间不足时明确提示无法安装本地 Qwen3-TTS，并提供千问AI平台云端入口。当前优先完成此设备的云端接入；用户填写 AK、启用云端后，每次生成确认发送选定参考录音和文本。无云端数据同步。详见[语音接入](docs/speech.md)。
...
![表达回顾界面，使用虚构示例](docs/screenshots/review-desktop.png)
...
| Skill 与表达评估 | 接入已实现 | 本机授权桥接、结构化评估、配置版本校验、去重；宿主自动触发待验证 |
...
| 云端 API | Qwen 接入已实现 | AK 本地保存、平台跳转、音色复用与合成；已完成一次真实云端合成 |

已通过 13 项数据、评估和语音流程测试与 Electron 端到端检查，覆盖录音、播放、归档、AK 本地保存/移除、平台跳转及退出重开后的持久化。录音测试使用模拟麦克风，不代表真实麦克风听感、Qwen 模型运行或自动客户端接入已验证。当前只在 macOS 做过桌面验证。
