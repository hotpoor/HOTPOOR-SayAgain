...
> Electron 源码客户端 v0.2：设置顶部提供多个 Qwen AK 的命名、查看、切换、本地明文保存和平台快捷链接；已接入音色克隆与合成任务流程、Skill 本地评估接口。云端通过模拟接口测试，真实付费合成与本地模型推理尚未验证；尚无安装包。
...
| 云端 API | Qwen 接入已实现 | AK 本地保存、平台跳转、音色复用与合成；尚未真实付费调用 |
