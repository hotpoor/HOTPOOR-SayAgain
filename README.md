...
> Electron 源码客户端 v0.2：设置顶部提供 Qwen AK 输入、系统加密保存和平台快捷链接；已接入音色克隆与合成任务流程、Skill 本地评估接口。云端通过模拟接口测试，真实付费合成与本地模型推理尚未验证；尚无安装包。
...
| 云端 API | Qwen 接入已实现 | AK 加密保存、平台跳转、音色复用与合成；尚未真实付费调用 |

已通过 13 项数据、评估和语音流程测试与 Electron 端到端检查，覆盖录音、播放、归档、AK 加密保存/移除、平台跳转及退出重开后的持久化。录音测试使用模拟麦克风，不代表真实麦克风听感、Qwen 模型运行或自动客户端接入已验证。当前只在 macOS 做过桌面验证。
