...
function renderSettings() {
  ...
  <section class="settings-section" id="cloud-settings"><h2>Qwen 云端 · 音色克隆与语音生成</h2><p>空间不足时，可主动选择千问AI平台云端。云端会发送选定参考录音和文本，按平台计费；不会自动切换。</p><form id="speech-form">${speech.api_key_error?`<div class="model-notice" role="alert">${escapeHtml(speech.api_key_error)}</div>`:''}<label class="field">生成方式<select name="mode"><option value="local">本地 Qwen3-TTS（优先）</option><option value="cloud">千问AI平台云端</option></select></label><div class="field"><label for="qwen-api-key">Qwen AK / API Key</label><span class="key-input"><input id="qwen-api-key" name="api_key" type="password" autocomplete="off" placeholder="${speech.has_api_key?'已保存，点击右侧眼睛查看当前 Key':'粘贴 Qwen 平台的 AK / API Key'}"><button type="button" class="icon-button key-visibility" data-action="toggle-api-key" aria-label="显示 API Key" aria-pressed="false" title="显示 API Key">${icon('eye')}</button></span><small>AK 明文保存在这台电脑的应用数据目录，不写入 SQLite 或备份。</small></div><label class="checkbox"><input name="cloud_enabled" type="checkbox" ${speech.config.body.cloud_enabled?'checked':''}>主动启用云端合成</label><p class="quiet-note">使用平台 API Key（AK），此接口不需要填写应用名称。保存设置不会发起付费请求。</p><p class="form-error" role="alert"></p><div class="form-actions"><button class="button primary" type="submit">保存语音设置</button><button class="button" type="button" data-action="cloud-platform">打开 Qwen 平台 ↗</button>${speech.api_key_present?'<button class="button quiet" type="button" data-action="clear-api-key">移除密钥</button>':''}</div></form></section>
...
document.addEventListener('click', async event => {
  ...
  try {
    ...
    switch(button.dataset.action) {
      ...
      case 'toggle-api-key': {
        const input=$('#qwen-api-key'),show=input.type==='password';
        if(show&&!input.value&&state.speech.has_api_key){
          button.disabled=true;
          try{const saved=await api.revealApiKey();if(!input.isConnected)return;if(!input.value)input.value=saved;}finally{button.disabled=false;}
        }
        input.type=show?'text':'password';button.innerHTML=icon(show?'eyeOff':'eye');
        button.setAttribute('aria-label',show?'隐藏 API Key':'显示 API Key');button.title=button.getAttribute('aria-label');button.setAttribute('aria-pressed',String(show));break;
      }
