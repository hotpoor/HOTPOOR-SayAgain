window.recordingPage=(()=>{
 const api=window.sayagain;let root,sessionId=null,searchQuery='',busy=false,stream,context,processor,source,parts=[],frames=0,offset=0,started=0,queue=Promise.resolve(),pending=0,failed=[],stopping=false,clipPage=0,speakerFilter="",pipelineTimer=null;
 ...
 function render(container,state){
  ...
  const allClips=(state.recording_clips||[]).filter(c=>c.body.recording_id===sessionId).sort((a,b)=>a.body.sequence-b.body.sequence);
  const filtered=allClips.filter(c=>!speakerFilter||(c.body.speaker_analysis?.segments?.[0]?.speaker||c.body.speaker||'unknown')===speakerFilter);clipPage=Math.min(clipPage,Math.max(0,Math.ceil(filtered.length/40)-1));const clips=filtered.slice(clipPage*40,clipPage*40+40);
  ...
  const confirmedNames=[...new Set(allClips.filter(c=>c.body.speaker_analysis?.status==='user_confirmed').flatMap(c=>c.body.speaker_analysis.segments.map(t=>t.speaker)).filter(n=>n!=='不确定'))];
  ...
  root.innerHTML=`<div class="page recordings-page">
   ...
   <p class="muted">已确认 ${confirmedNames.length} 位说话人：${confirmedNames.map(esc).join('、')||'尚未确认'} · 未确认片段 ${allClips.filter(c=>c.body.speaker_analysis?.status!=='user_confirmed').length} 个</p><div class="recording-toolbar"><select id="clip-speaker" aria-label="筛选说话人"><option value="">全部说话人</option>${[...new Set(allClips.filter(c=>c.body.pipeline_version).map(c=>c.body.speaker_analysis?.segments?.[0]?.speaker||c.body.speaker||'unknown'))].sort().map(n=>`<option value="${n}" ${n===speakerFilter?'selected':''}>${n==='unknown'?'不确定':'说话人 '+n}</option>`).join('')}</select><button class="button" id="clips-prev" ${clipPage===0?'disabled':''}>上一页</button><span>${filtered.length} 条 · 第 ${clipPage+1} / ${Math.max(1,Math.ceil(filtered.length/40))} 页</span><button class="button" id="clips-next" ${(clipPage+1)*40>=filtered.length?'disabled':''}>下一页</button></div><div id="recording-clips">${clips.length?`<div class="recordings-list-heading"><h2>录音片段</h2><span>按录制顺序</span></div>${clips.map(c=>clipView(c)).join('')}`:`<div class="recordings-empty"><div class="recordings-empty-wave" aria-hidden="true">${Array.from({length:7},()=>'<i></i>').join('')}</div><h2>${sessionId?'从一段对话开始':'给对话留一个位置'}</h2><p>${sessionId?'点击「开始录音」，或导入已有音频。':'新建一个会话，再录音或导入音频。'}<br>你的声音和笔记会保存在这里。</p></div>`}</div>
  ...
  bindTurns();bindPipeline();
  root.querySelector('#clips-prev').onclick=()=>{clipPage--;render(root,state);};
  root.querySelector('#clips-next').onclick=()=>{clipPage++;render(root,state);};
  root.querySelector('#clip-speaker').onchange=e=>{speakerFilter=e.target.value;clipPage=0;render(root,state);};
 ...
 function renderList(sessions,clips,sources){
  ...
  list.onclick=async e=>{const row=e.target.closest('[data-recording-open]');if(!row)return;sessionId=row.dataset.recordingOpen;clipPage=0;speakerFilter="";await reload();};
 ...
 async function startPipeline(action){
  const options=pipelineOptions();busy=true;lock(true);status('正在准备本地分人拆条…');
  try{const job=await api.speakerPipeline({action,id:sessionId,options});if(!job){busy=false;await reload();return;}watchPipeline();}
 ...
 function watchPipeline(){
  ...
  const labels={starting:'准备模型',decode:'解码音频',voiceprints:'检测语音与提取声纹',cluster:'建立说话人分组',transcribe:'生成候选语音条',complete:'保存结果'};
  const poll=async()=>{try{
   const job=await api.speakerPipeline({action:'status'});
   if(job.state==='running'){const cancel=root?.querySelector('#pipeline-cancel');if(cancel)cancel.hidden=false;status((labels[job.stage]||'处理中')+(job.total?` · ${job.done}/${job.total}`:'')+'，可取消；原始数据不会改变。');return;}
   clearInterval(pipelineTimer);pipelineTimer=null;busy=false;
   if(job.state==='completed'){sessionId=job.recording_id;clipPage=0;speakerFilter='';await reload();status(`已生成 ${job.clip_count} 条分人语音，${job.review_count} 条建议复核声音归属。请试听并确认时间段与说话人，然后转写。`);}
   else{await reload();status(job.state==='cancelled'?'已取消，未导入部分结果。':job.error||'任务已结束');}
 ...
 function lock(value){root.querySelectorAll('#pipeline-import,#pipeline-session,#pipeline-language,#pipeline-count,#recording-search,#recording-create,[data-recording-open],#clips-prev,#clips-next,#clip-speaker,#recording-back,#recording-start,#recording-import,#recording-speakers,#recording-detect,#recording-resegment,#recording-clear-analysis,#recording-reanalyze,[data-save-transcript],[data-transcribe],[data-transcript],[data-turn-editor] input,[data-turn-editor] button').forEach(el=>el.disabled=value);const stop=root.querySelector('#recording-stop');if(stop){stop.disabled=!stream;stop.hidden=!stream;}if(!value&&sessionId){root.querySelector('#recording-speakers').disabled=!root.querySelector('.recording-clip');root.querySelector('#recording-detect').disabled=!root.querySelector('.recording-clip');root.querySelectorAll('[data-turn-editor]').forEach(e=>{e.closest('.recording-clip').querySelector('[data-transcribe]').disabled=!e.querySelector('summary').textContent.includes('已确认')||!!e.querySelector('[data-turn-state]').textContent;});}}
