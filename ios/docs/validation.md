...
## 2026-09-27 · iPhone 12 Pro 同音频实测

现代开发签名安装与四轮完整转写/重启恢复测试均通过。Release 两轮总耗时 69.22 / 69.15 秒，真正 `-Onone` 的 Debug 对照 73.88 秒；另有一轮实际仍为 `-O` 的 Debug 命名配置 68.95 秒，已明确归类，不计入未优化对照。后两轮开始/结束低电量模式均为 true、系统热状态 serious。

具体输入哈希、构建条件、原始结果包位置、限制与深色模式对比截图见 [iPhone 12 Pro 性能记录](iphone12pro-benchmark.md)。修复白底白字并增加可选电源/热状态字段后，旧 Mac 的 iOS 12 目标编译也通过；没有重新跑 iPad 的四分钟推理。

最终 Release 重新安装成功，常用草稿回读与 Release 实测 JSON 完全一致；同音频 WAV、资料库和两份独立报告仍在 Documents。最后自动打开时设备已锁屏，被系统拒绝启动；之前各轮测试已完成实际启动与推理，不能把这次锁屏拒绝写成模型或构建失败。
